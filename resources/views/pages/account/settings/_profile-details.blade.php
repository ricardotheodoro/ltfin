<!--begin::Basic info-->
@php
    $accountInfo = $info ?? auth()->user()->info;
@endphp
            ...
            <div class="card-body border-top p-9">
                ...
                @if($errors->any())
                    <div class="alert alert-danger d-flex align-items-center p-5 mb-10">
                        ...
                        <div class="d-flex flex-column">
                            <span>{{ __('Verifique os dados informados e tente novamente.') }}</span>
                        </div>
                    </div>
                @endif
                ...
                <div class="row mb-6">
                    ...
                    <div class="col-lg-8">
                        <!--begin::Image input-->
                        <div class="image-input image-input-outline {{ !empty($accountInfo->avatar) ? '' : 'image-input-empty' }}" data-kt-image-input="true" style="background-image: url({{ asset(theme()->getMediaUrlPath() . 'avatars/blank.png') }})">
                            <!--begin::Preview existing avatar-->
                            <div class="image-input-wrapper w-125px h-125px" style="background-image: {{ !empty($accountInfo->avatar_url) ? 'url('.asset($accountInfo->avatar_url).')' : 'none' }};"></div>
                ...
                <div class="row mb-6">
                    ...
                    <div class="col-lg-8 fv-row">
                        <input type="text" name="company" class="form-control form-control-lg form-control-solid" placeholder="Company name" value="{{ old('company', $accountInfo->company ?? '') }}"/>
                    </div>
                    <!--end::Col-->
                </div>
                <!--end::Input group-->
                ...
                <div class="row mb-6">
                    ...
                    <!--begin::Col-->
                    <div class="col-lg-8 fv-row">
                        <input type="tel" name="phone" class="form-control form-control-lg form-control-solid" placeholder="Phone number" value="{{ old('phone', $accountInfo->phone ?? '') }}"/>
                    </div>
                    <!--end::Col-->
                </div>
                <!--end::Input group-->
                ...
                <div class="row mb-6">
                    ...
                    <!--begin::Col-->
                    <div class="col-lg-8 fv-row">
                        <input type="text" name="website" class="form-control form-control-lg form-control-solid" placeholder="Company website" value="{{ old('website', $accountInfo->website ?? '') }}"/>
                    </div>
                    <!--end::Col-->
                </div>
                <!--end::Input group-->
                    ...
                    <div class="col-lg-8 fv-row">
                        <select name="country" aria-label="{{ __('Select a Country') }}" data-control="select2" data-placeholder="{{ __('Select a country...') }}" class="form-select form-select-solid form-select-lg fw-bold">
                            ...
                            @foreach(\App\Core\Data::getCountriesList() as $key => $value)
                                <option data-kt-flag="{{ $value['flag'] }}" value="{{ $key }}" {{ $key === old('country', $accountInfo->country ?? '') ? 'selected' :'' }}>{{ $value['name'] }}</option>
                            @endforeach
                        </select>
                    </div>
                    <!--end::Col-->
                    ...
                    <div class="col-lg-8 fv-row">
                        ...
                        <select name="language" aria-label="{{ __('Select a Language') }}" data-control="select2" data-placeholder="{{ __('Select a language...') }}" class="form-select form-select-solid form-select-lg">
                            ...
                            @foreach(\App\Core\Data::getLanguagesList() as $key => $value)
                                <option data-kt-flag="{{ $value['country']['flag'] }}" value="{{ $key }}" {{ $key === old('language', $accountInfo->language ?? '') ? 'selected' :'' }}>{{ $value['name'] }}</option>
                            @endforeach
                        </select>
                        <!--end::Input-->
                    ...
                    <div class="col-lg-8 fv-row">
                        <select name="timezone" aria-label="{{ __('Select a Timezone') }}" data-control="select2" data-placeholder="{{ __('Select a timezone..') }}" class="form-select form-select-solid form-select-lg">
                            ...
                            @foreach(\App\Core\Data::getTimeZonesList() as $key => $value)
                                <option data-bs-offset="{{ $value['offset'] }}" value="{{ $key }}" {{ $key === old('timezone', $accountInfo->timezone ?? '') ? 'selected' :'' }}>{{ $value['name'] }}</option>
                            @endforeach
                        </select>
                    </div>
                    <!--end::Col-->
                    ...
                    <div class="col-lg-8 fv-row">
                        <select name="currency" aria-label="{{ __('Select a Currency') }}" data-control="select2" data-placeholder="{{ __('Select a currency..') }}" class="form-select form-select-solid form-select-lg">
                            ...
                            @foreach(\App\Core\Data::getCurrencyList() as $key => $value)
                                <option data-kt-flag="{{ $value['country']['flag'] }}" value="{{ $key }}" {{ $key === old('currency', $accountInfo->currency ?? '') ? 'selected' :'' }}><b>{{ $key }}</b>&nbsp;-&nbsp;{{ $value['name'] }}</option>
                            @endforeach
                        </select>
                    </div>
                    <!--end::Col-->
                    ...
                    <div class="col-lg-8 fv-row">
                        ...
                        <div class="d-flex align-items-center mt-3">
                            ...
                            <label class="form-check form-check-inline form-check-solid me-5">
                                ...
                                <input class="form-check-input" name="communication[email]" type="checkbox" value="1" {{ old('communication.email', data_get($accountInfo, 'communication.email', '')) ? 'checked' : '' }}/>
                            ...
                            <label class="form-check form-check-inline form-check-solid">
                                ...
                                <input class="form-check-input" name="communication[phone]" type="checkbox" value="1" {{ old('communication.phone', data_get($accountInfo, 'communication.phone', '')) ? 'checked' : '' }}/>
                                <span class="fw-bold ps-2 fs-6">
                                    {{ __('Phone') }}
                                </span>
                            </label>
                ...
                <div class="row mb-0">
                    ...
                    <div class="col-lg-8 d-flex align-items-center">
                        <div class="form-check form-check-solid form-switch fv-row">
                            ...
                            <input class="form-check-input w-45px h-30px" type="checkbox" id="allowmarketing" name="marketing" value="1" {{ old('marketing', $accountInfo->marketing ?? '') ? 'checked' : '' }}/>
