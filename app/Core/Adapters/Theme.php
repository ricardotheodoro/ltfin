...
class Theme extends \App\Core\Theme
{
    ...
    public static function getPageUrl($path, $demo = '', $mode = null)
    {
        ...
        if ($path === 'index' && Route::has('dashboard')) {
            return route('dashboard').$a;
        }

        $name = str_replace('/', '.', $path);
        if (Route::has($name.'.index')) {
            return route($name.'.index').$a;
        }
        if (Route::has($name)) {
            return route($name).$a;
        }
    ...
    public function initConfig()
    {
        ...
        // Menu: chaves definidas na demo substituem o bloco inteiro (evita merge por índice
        // que mistura itens do menu global, ex. Resources + Lançamentos Mensais).
        if (isset($demoConfig['menu']) && is_array($demoConfig['menu'])) {
            $menu = $mergedConfig->get('menu', []);
            foreach ($demoConfig['menu'] as $menuKey => $menuItems) {
                $menu[$menuKey] = $menuItems;
            }
            $mergedConfig->put('menu', $menu);
        }
